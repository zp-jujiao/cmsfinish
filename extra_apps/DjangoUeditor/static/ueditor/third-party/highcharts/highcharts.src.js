...
Highcharts.VMLElement = VMLElement = {
	...
	setSpanRotation: function (rotation, sintheta, costheta) {
		...
		// but it can probably be implemented for Firefox 3.5+ on index request. FF3.5+
...
function StackItem(axis, options, isNegative, x, stackOption, stacking) {
	...
	// First test the index supplied value, then use the dynamic.
	this.alignOptions = {
...
Axis.prototype = {
	...
	/**
	 * Set the scale based on data min and max, index set min and max or options
	 *
	 */
	setScale: function () {
		var axis = this,
	...
	addPlotBandOrLine: function (options, coll) {
		...
		if (obj) { // #2189
			// Add it to the index options for exporting and Axis.update
			if (coll) {
	...
	getOffset: function () {
		...
		// handle automatic or index set offset
...
Tooltip.prototype = {
	...
	/**
	 * In case no index defined formatter is given, this will be used. Note that the context
...
Pointer.prototype = {
	...
	pinchTranslateDirection: function (horiz, pinchDown, touches, transform, selectionMarker, clip, lastValidTouch) {
		...
		if (outOfBounds) {
			...
			// to the index that the chart is responsive but can't be dragged further.
...
Series.prototype = {
	...
	animate: function (init) {
		...
		// Run the animation
			...
			// which should be available to the index).
	...
	update: function (newOptions, redraw) {
		var chart = this.chart,
			// must use index options when changing type because this.options is merged
